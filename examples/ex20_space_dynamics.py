...
back = anvil.R.cartesian_to_keplerian(
    r_vec=eci['r_eci'], v_vec=eci['v_eci'], mu=mu_E,
)
print(f"  Round-trip:  a = {back['a']}  e = {back['e']:.5f}")
print(f"               i = {back['i_deg']:.3f} deg  RAAN = {back['RAAN_deg']:.3f} deg")
...
for step in range(11):
    if step % 2 == 0:
        print(f"  {step*dt:>4.0f}  {q[0]:>8.5f}  {q[1]:>8.5f}  {q[2]:>8.5f}  {q[3]:>8.5f}  "
              f"{sum(x**2 for x in q)**0.5:>6.4f}")
    if step < 10:
        qd = anvil.R.quaternion_kinematics(
            q_w=q[0], q_x=q[1], q_y=q[2], q_z=q[3],
            omega_x=0.0, omega_y=0.01, omega_z=0.0,
        )
...
print(f"  True rotation: 30 deg about Z  ->  q_z = sin(15 deg) = {np.sin(np.radians(15)):.5f}")
print(f"  TRIAD q: w={tr['q_w']:.5f}  x={tr['q_x']:.5f}  y={tr['q_y']:.5f}  z={tr['q_z']:.5f}")
...
pwr = anvil.R.power_budget(
    P_load_W=80, T_orbit_min=94.6,
    eclipse_frac=ecl_worst['eclipse_frac'],
    eta_solar=0.30, flux_solar=1361.0, DOD=0.8, eta_battery=0.9,
)
...
pwr_sys.add("P_load_W", 80); pwr_sys.add("T_orbit_min", 94.6)
pwr_sys.add("eclipse_frac", 0.35); pwr_sys.add("eta_solar", 0.30)
pwr_sys.add("flux_solar", 1361.0); pwr_sys.add("DOD", 0.8)
...
lnk = anvil.R.link_budget(
    P_tx_W=5, G_tx_dBi=3, G_rx_dBi=47, freq_Hz=8.4e9,
    distance_m=a_SSO, losses_dB=4.0,
)
...
for d_km in [400, 600, 800, 1200, 2000]:
    r = anvil.R.link_budget(P_tx_W=5, G_tx_dBi=3, G_rx_dBi=47,
                             freq_Hz=8.4e9, distance_m=d_km*1e3, losses_dB=4.0)
...
print(f"  Overshoot: {m['overshoot_pct']:.1f}%   t_settle: {m['t_settle']:.1f} s   t_rise: {m['t_rise']:.1f} s")

poles = anvil.R.state_space_poles(A_flat=[0, 1, -Kp/Iz, -Kd/Iz], n_states=2)
print(f"  Poles: {[complex(round(r,4),round(i,4)) for r,i in zip(poles['poles_real'],poles['poles_imag'])]}")
...
q_lqr = anvil.R.lqr_bryson(
    state_bounds=[np.radians(5)]*3, input_bounds=[0.5]*3,
)
print(f"  LQR Q = {[round(x,1) for x in q_lqr['Q_diag']]}  R = {[round(x,2) for x in q_lqr['R_diag']]}")
...
for r_b_km in [100_000, 200_000, 384_400]:
    be = anvil.R.bielliptic_transfer(mu=mu_E, r1=R_E+400e3, r2=42164e3, rb=r_b_km*1e3)
    print(f"  Bi-elliptic rb={r_b_km:7d} km:  dv = {be['dv_total'].to("km/s")}   tof = {be['tof']}")
